...
EVENT_TOL = 5
SOC_TRACE_MAX_AGENTS = 128 if ENABLE_C else 0
...
UPSTREAM_MASK = build_upstream_mask(5.0) if ENABLE_B else None
...
PAYLOAD_FIELDS = BASE_FIELDS + A_FIELDS + (B_FIELDS if ENABLE_B else []) + (C_FIELDS if ENABLE_C else [])
...
INT_FIELDS = {
    ...
    'g_down_at_decision', 'post_g_down_count', 'pre_I_soc_count'
}
...
sampled_agent_ids = set(np.linspace(0, N_AGENTS - 1, SOC_TRACE_MAX_AGENTS, dtype=int).tolist()) if ENABLE_C and SOC_TRACE_MAX_AGENTS > 0 else set()
ring_buffers = {int(aid): deque(maxlen=SOC_TRACE_PRE_WINDOW) for aid in arrays['agent_id'] if int(aid) in sampled_agent_ids} if ENABLE_C else {}
social_trace_rows_local = []

output_dir = f"simulation_results_threshold_10m_{PRIORITY.lower()}"
history_log = []
if rank == 0:
...
for frame in range(NUM_FRAMES):
    ...
    for step in range(MAX_RELAX_STEPS):
        ...
        for k in range(len(arrays['x'])):
            ...
            if arrays['s'][k] == -1:
                ...
                continue
            if ENABLE_B and arrays['step_decided'][k] >= 0 and arrays['evac_time'][k] == -1:
    ...
    gathered_social_trace = comm.gather(social_trace_rows_local if ENABLE_C else [], root=0)
    if ENABLE_C:
        social_trace_rows_local = []
    ...
    if rank == 0:
            ...
            for idx in range(n):
                ...
                if ENABLE_B:
                    row.update({
                        ...
                        'post_g_down_mean': float(part['post_g_down_sum'][idx] / part['post_g_down_count'][idx]) if part['post_g_down_count'][idx] > 0 else np.nan,
                    })
                if ENABLE_C:
                    c = int(part['pre_I_soc_count'][idx])
                    mean_soc = float(part['pre_I_soc_sum'][idx] / c) if c > 0 else np.nan
                    var_soc = float(part['pre_I_soc_sq_sum'][idx] / c - mean_soc**2) if c > 0 else np.nan
                    row.update({
                        'pre_I_soc_mean': mean_soc,
                        'pre_I_soc_var': var_soc,
                        'pre_I_soc_max': float(part['pre_I_soc_max'][idx]) if c > 0 else np.nan,
                        'pre_I_soc_min': float(part['pre_I_soc_min'][idx]) if c > 0 else np.nan,
                        'pre_I_soc_count': c,
                    })
        ...
        rec = {
            ...
            'A_dec_per_step': A_dec_frame / steps_used_this_frame,
            'A_dec_per_h': A_dec_frame / H_STEP,
            'J_out_frame': J_out_frame,
            'J_out_per_step': J_out_frame / steps_used_this_frame,
            'J_out_per_h': J_out_frame / H_STEP,
            'Q_transport': Q_transport,
            'q_transport': q_transport,
        }
        if ENABLE_B:
            rec.update({'rho_up': rho_up, 'Q_up': Q_up})
        history_log.append(rec)
        print(f"[Priority {PRIORITY} | Frame {frame:03d}/{NUM_FRAMES-1}] h={current_h_ext:.3f}, m_dec_cum={m_dec_cum:.4f}, m_evac_cum={m_evac_cum:.4f}, Q={Q_transport}, steps={steps_used_this_frame}")

print(f"Time: {time.time() - start}s")
if rank == 0:
    pd.DataFrame(history_log).to_csv(os.path.join(output_dir, 'macro_stats.csv'), index=False)
    print(f"=== 全計算プロセスが完了しました（Priority {PRIORITY}） ===")
